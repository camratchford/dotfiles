...
def draw_title(data: dict) -> str:
    ...
    try:
        active_wd = getattr(data.get("tab", {}), "active_wd", None)
        if active_wd and active_wd in title:
            name_and_parent = f"{Path(active_wd).parent.name}/{Path(active_wd).name}"
            title = re.sub(active_wd, name_and_parent, title)
